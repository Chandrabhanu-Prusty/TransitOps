      ...
      <div className="border-t border-slate-800 p-2 space-y-1 shrink-0">
        {!collapsed && (
          <div className="flex items-center gap-2.5 px-2 py-2">
            ...
            <button onClick={handleLogout} className="p-1 hover:bg-slate-800 rounded transition-colors" title="Log out">
...
type PageProp = "login" | "register" | Screen;

export default function App({ initialPage }: { initialPage: PageProp }) {
  const [collapsed, setCollapsed] = useState(false);

  // Derive role from JWT user stored in localStorage
  const loggedInUser = authService.getUser();
  const roleRaw = loggedInUser?.role ?? "DISPATCHER";
  ...
  const screen = initialPage as Screen;
